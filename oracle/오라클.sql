...
-- primary keym PK, 주요키, 중요키, 기본키
-- not null + unique 조건
-- 생성과 동시에 index도 생성해줌
-- create table에서는 primary key를 딱 하나만 지정
-- 두개 이상의 컬럼을 primary key로 지정 하려면 alter사용
...
create table emp_fk(
    ...
    -- deptno number references dept_fk -- 컬럼명이 같다면 생략 가능
);

insert into dept_fk
values (100,'1강의실');
insert into emp_fk values (1,'이름',101);-- dept_fk 에 없는 101은 들어갈수 없다
